...
import {
    ...
    TrendingUp, Wallet, UserCheck, Filter, Globe, Smartphone
} from 'lucide-react';

// --- 1. CHART GARIS (REVENUE) - VERSI CLEAN & ELEGANT ---
const SimpleLineChart = ({ data }: { data: number[] }) => {
    // Tentukan Max Value biar grafik tidak mentok atap (kasih buffer 20%)
...
// --- 2. CHART BATANG (CHANNEL) - FIX UKURAN BIAR GAK GEDE ---
const SimpleBarChart = ({ data, labels }: { data: number[], labels: string[] }) => {
    const max = Math.max(...data, 1);
    // Warna khusus per channel biar cantik
    ...
    return (
        <div className="h-64 w-full flex flex-col justify-end">
            {/* Area Chart: Pakai justify-center biar kalau cuma 1 bar dia di tengah, bukan melebar */}
            <div className="flex items-end justify-center gap-8 h-full pb-2 border-b border-gray-100">
                {data.map((val, i) => (
                    <div key={i} className="flex flex-col items-center group relative h-full justify-end w-16"> {/* w-16 KUNCI BIAR GAK LEBAR */}
                        {/* Tooltip Angka */}
                        <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity bg-keenan-dark text-white text-[10px] px-2 py-1 rounded shadow-lg transform translate-y-2 group-hover:translate-y-0">
                            {val} Booking
                        </div>

                        {/* Batang Chart */}
                        <div
                            className={`w-12 rounded-t-md transition-all duration-500 ease-out hover:opacity-90 ${getBarColor(labels[i])}`}
                            style={{ height: `${Math.max((val / max) * 100, 2)}%` }} // Minimal 2% biar kelihatan dikit
                        ></div>

                        {/* Label Channel */}
                        <div className="mt-3 text-center">
                            <span className="text-[10px] font-bold text-gray-500 uppercase block truncate max-w-[60px]" title={labels[i]}>
                                {labels[i]}
                            </span>
                        </div>
                    </div>
...
export default function SuperAdminDashboard() {
    ...
    const calculateAnalytics = () => {
        ...
        // --- FIX VISUALISASI JIKA DATA KOSONG ---
        // Biar chart gak blank putih jelek
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex font-sans text-gray-800">
            ...
            <div className="flex-1 md:ml-64 p-8 lg:p-12">
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                                <div><p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Total Revenue</p><p className="text-3xl font-bold text-keenan-dark">{formatRupiah(analytics.totalRevenue)}</p></div>
                                <div className="w-12 h-12 bg-green-100 text-green-600 rounded-full flex items-center justify-center"><Wallet /></div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                                <div><p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Total Bookings</p><p className="text-3xl font-bold text-keenan-gold">{analytics.totalBookings}</p></div>
                                <div className="w-12 h-12 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center"><UserCheck /></div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                                <div><p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Managed Properties</p><p className="text-3xl font-bold text-blue-600">{selectedPropertyFilter === 'all' ? properties.length : 1}</p></div>
                                <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center"><Hotel /></div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                            ...
                            <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                <div className="flex justify-between items-center mb-6">
                                    <h3 className="font-bold text-lg text-keenan-dark flex items-center gap-2"><TrendingUp size={20} className="text-keenan-gold" /> Revenue Trend</h3>
                                    <span className="text-xs font-bold text-gray-400 bg-gray-100 px-3 py-1 rounded-full">Current Year</span>
                                </div>
                                <SimpleLineChart data={analytics.monthlyRevenue} />
                            </div>

                            {/* CHANNEL STATS (FIXED UI) */}
                            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col">
                                <div className="mb-auto">
                                    <h3 className="font-bold text-lg text-keenan-dark">Booking Channel</h3>
                                    <p className="text-xs text-gray-400">Where do your guests come from?</p>
                                </div>
                                {analytics.channelStats.data.length > 0 ? (
                                    <SimpleBarChart data={analytics.channelStats.data} labels={analytics.channelStats.labels} />
                                ) : (
                                    <div className="flex-1 flex flex-col items-center justify-center text-gray-300">
                                        <Globe size={48} className="mb-2 opacity-20" />
                                        <p className="text-sm italic">No channel data available</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                                ...
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-gray-50 text-gray-400 uppercase text-[10px] font-bold">
                                        <tr><th className="p-6">Rank</th><th className="p-6">Property Name</th><th className="p-6 text-right">Total Revenue</th></tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-50">
                                        {analytics.topProperties.map(([name, revenue], index) => (
                                            <tr key={name} className="hover:bg-gray-50">
                                                <td className="p-6 font-bold text-keenan-gold">#{index + 1}</td>
                                                <td className="p-6 font-bold text-keenan-dark">{name}</td>
                                                <td className="p-6 text-right font-mono font-bold text-green-600">{formatRupiah(revenue)}</td>
                                            </tr>
                                        ))}
                                        {analytics.topProperties.length === 0 && (
                                            <tr><td colSpan={3} className="p-8 text-center text-gray-400 italic">No revenue data available.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                ...
                {/* --- RENDER TAB LAIN (Properties, Rooms, Staff) SAMA SEPERTI SEBELUMNYA --- */}
                ...
                {/* ... Paste juga bagian ROOMS & STAFF dari kode sebelumnya di sini ... */}
